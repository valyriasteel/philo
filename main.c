...
{
    t_philosophers *philo = (t_philosophers *)value;
...
int main(int ac, char **av)
{
    ...
    while (++i < philomain.philo_nb)
    {
        ...
        pthread_create(&philomain.threads[i], NULL, my_thread, &philomain.philos[i]);
    }
    i = -1;
    while (1)
    {
        ...
        {
            if (gettime() - philomain.philos[i].end_time_to_eat >= philomain.time_to_die)
            {
